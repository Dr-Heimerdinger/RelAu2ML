...
def _get_kaggle_api():
    """Authenticate and return a KaggleApi instance, or raise with a clear message."""
    try:
        from kaggle.api.kaggle_api_extended import KaggleApi
    except ImportError:
        raise RuntimeError("kaggle package is not installed")
    api = KaggleApi()
    api.authenticate()
    return api
...
def search_gnn_competitions_for_benchmarks(
    ...
    try:
        ...
        return {
            ...
            "error": f"Kaggle authentication failed: {e}. Set KAGGLE_USERNAME and KAGGLE_KEY environment variables.",
        }
...
def search_kaggle_datasets(query: str, limit: int = 5):
    ...
    try:
        ...
        return [{"error": f"Kaggle authentication failed: {e}"}]
    ...
    try:
        ...
        return [{"error": f"Kaggle API error: {e}"}]
...
def download_kaggle_dataset(dataset_ref: str, path: str = "data"):
    ...
    try:
        ...
        return f"Kaggle authentication failed: {e}"
